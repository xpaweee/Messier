...
namespace Messier.Messaging.RabbitMQ.Conventions;
...
internal sealed class CustomConventions : IConventions
{
    ...
    public CustomConventions()
    {
     ...
     private static string GetMessageKey( string messageType) 
         => messageType.Underscore();
     private static string GetTypeKey(MemberInfo type) 
         => GetMessageKey(type.Name);
